...
[中文说明](README.zh-CN.md) · [Product site](https://wps.file-viewer.app) · [Standalone demo](https://wps.file-viewer.app/demo) · [Commercial license](https://dev.flyfish.group/shop/item/1016)
...
The same package tarball is attached to the GitHub v0.2.0 release. Publishing
...
const url = createWpsViewerUrl({
  viewerUrl: 'https://office.example.com/wps-viewer/demo',
  source: {
...
The product site lives at `https://wps.file-viewer.app` and embeds the separately
maintained evaluator from `/demo`. The public deployment remains limited to
local-file evaluation; production embedding requires the same `/demo` route on
a self-hosted, licensed runtime.
